...
def generate_html_report(data: Dict[str, Any]) -> str:
    ...
    if signals:
        for s in signals[:10]:
            ...
            reps_html = "".join([
                f'<div style="background:#050505; border:1px solid #1f1f23; padding:8px 12px; margin-bottom:6px; border-radius:8px; font-family:monospace; font-size:11px; color:#a1a1aa;">"{msg}"</div>'
        ...
        signals_html = '<div style="background:#0c0c10; border:1px solid #27272a; padding:25px; text-align:center; color:#71717a; border-radius:16px; font-size:12px; font-family:monospace;">Zero signals detected during this session window. Clean broadcast baseline.</div>'

    tot_msgs = stream.get('total_messages', 0)
    tot_sigs = stream.get('total_signals', 0)
    ratio = f"{round(tot_msgs / tot_sigs)}:1" if tot_msgs > 0 and tot_sigs > 0 else "—"
    ...
    distribution_bar_html = '<div style="display:flex; height:18px; border-radius:9px; overflow:hidden; margin-bottom:15px; border:1px solid #27272a;">'
    ...
    for cat_key, meta in cat_styles.items():
        ...
        if count > 0:
            distribution_bar_html += f'<div style="width:{pct}%; background:{meta["text"]}; height:100%;"></div>'
            legend_html += f"""
    ...
    if not category_counts:
        distribution_bar_html = '<div style="background:#18181b; height:10px; border-radius:5px; margin-bottom:10px;"></div>'
        legend_html = '<div style="color:#71717a; font-size:11px; font-family:monospace; margin-bottom:20px;">Zero category friction recorded.</div>'

    # Audience DNA Counts
    champs = dna.get('champions', 0)
    learns = dna.get('learners', 0)
    casuals = dna.get('casuals', 0)
    trolls = dna.get('trolls', 0)
    if champs == 0 and learns == 0 and casuals == 0:
        casuals = stream.get('unique_participants', 1)

    return f"""
    <html>
    <head>
        <title>PULSE Forensic Session Report</title>
        <style>
            @page {{ size: A4; margin: 15mm; }}
            body {{ font-family: sans-serif; background: #050505; color: #ededed; margin: 0; padding: 25px; }}
            ...
            .stat-card {{ background: #0c0c10; border: 1px solid #27272a; padding: 14px; border-radius: 12px; }}
            .stat-label {{ font-size: 8px; color: #71717a; text-transform: uppercase; letter-spacing: 1px; font-weight: bold; margin-bottom: 4px; }}
            .stat-value {{ font-size: 22px; font-weight: bold; color: #ffffff; font-family: monospace; }}
            .score-card {{ background: {c['bg']}; border: 1px solid {c['border']}; border-radius: 20px; padding: 20px; display: flex; align-items: center; justify-content: space-between; }}
            .score-circle {{ width: 90px; height: 90px; border-radius: 50%; border: 6px solid {c['stroke']}; display: flex; flex-direction: column; align-items: center; justify-content: center; }}
            .dna-card {{ background: #0c0c10; border: 1px solid #27272a; border-radius: 20px; padding: 20px; }}
            .dna-row {{ display: flex; justify-content: space-between; align-items: center; padding: 8px 12px; margin-bottom: 8px; border-radius: 8px; font-size: 11px; }}
        </style>
    </head>
    <body>
        <div class="header">
            <div>
                <h2 style="color:#a855f7; margin:0; font-size:18px; letter-spacing:1px; font-family:monospace;">PULSE FORENSIC AUTOPSY</h2>
                <p style="color:#71717a; margin:4px 0 0 0; font-size:10px; font-family:monospace;">Engine Standard · National SIH 2026</p>
            </div>
            <div style="text-align: right;">
                <h1 style="color:white; margin:0; font-size:18px; font-weight:bold;">{stream.get('title', 'Session')}</h1>
                <p style="color:#71717a; margin:4px 0 0 0; font-size:10px; font-family:monospace;">Compiled: {datetime.utcnow().strftime('%Y-%m-%d · %H:%M UTC')}</p>
            </div>
        </div>
        ...
        <div class="stats-bar">
            <div class="stat-card">
                ...
                <div class="stat-value">{tot_msgs}</div>
            </div>
            <div class="stat-card">
                <div class="stat-label" style="color:#c084fc;">Pure Signals</div>
                <div class="stat-value" style="color:#c084fc;">{tot_sigs}</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">Unique Voices</div>
                <div class="stat-value">{stream.get('unique_participants', 1)}</div>
            </div>
            <div class="stat-card">
        ...
        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px; margin-bottom:25px;">
            <div class="score-card">
                <div class="score-circle">
                    <span style="font-size:32px; font-weight:bold; color:{c['text']}; font-family:monospace;">{score.get('score', 50)}</span>
                    <span style="font-size:8px; color:#71717a; font-family:monospace;">/ 100</span>
                </div>
                <div style="flex:1; margin-left:20px;">
                    <span style="display:inline-block; background:{c['bg']}; border:1px solid {c['border']}; padding:4px 10px; border-radius:20px; font-size:10px; font-weight:bold; color:{c['text']}; text-transform:uppercase; letter-spacing:1px; margin-bottom:12px;">
                        {score.get('label', 'ACTIVE SESSION')}
                    </span>
                    {comp_html}
                </div>
            </div>
            ...
            <div class="dna-card">
                <h3 style="color:#71717a; font-size:10px; text-transform:uppercase; letter-spacing:1px; margin:0 0 12px 0;">Audience DNA</h3>
                ...
                <div class="dna-row" style="background:rgba(113, 113, 122, 0.1); border:1px solid rgba(113, 113, 122, 0.25); color:#d4d4d8;">
                    ...
                    <strong>{casuals}</strong>
                </div>
            </div>
        </div>

        <h2 style="color:#71717a; font-size:10px; text-transform:uppercase; letter-spacing:1px; margin-bottom:8px;">Semantic Category Distribution</h2>
        {distribution_bar_html}
        {legend_html}

        <h2 style="color:#71717a; font-size:10px; text-transform:uppercase; letter-spacing:1px; margin-bottom:12px;">Extracted Autopsy Signals</h2>
        {signals_html}
    </body>
    </html>
    """
